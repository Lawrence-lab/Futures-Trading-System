...
class GatekeeperBNFB5mStrategy:
    def __init__(self, name="Gatekeeper_BNF_B_5m", portfolio=None, contract=None):
        """
    ...
    def check_signals(self, df_5m, df_1d=None, precalc_bullish_1d=None, precalc_signal_60m=None):
        ...
        # --- 指標計算 ---
        # 取得 60MA 與 Bias
        sma_series = calculate_sma(df_5m, period=self.sma_period)
        bias_series = calculate_bias(df_5m, sma_col=None, period=self.sma_period)
        ...
        # 取得 Volume MA
        vol_ma_series = df_5m['volume'].rolling(window=self.volume_ma_period).mean()
